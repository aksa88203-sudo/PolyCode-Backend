...
// Path to your Rust folders
const RUST_BASE_PATH = path.join(__dirname, '../../../data');
...
async function checkDataStructure() {
  try {
    ...
    if (await fs.access(RUST_BASE_PATH).then(() => true).catch(() => false)) {
      console.log('✓ Data folder found at:', RUST_BASE_PATH);
    } else {
      console.log('✗ Data folder not found at:', RUST_BASE_PATH);
    }
  } catch (error) {
...
async function scanDirectory(dirPath, basePath = RUST_BASE_PATH, maxDepth = 3, currentDepth = 0) {
  const items = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (['.rs', '.md', '.txt', '.js', '.html', '.css'].includes(ext)) {
...
router.get('/', async (req, res) => {
  try {
    ...
    // Scan data directory
    if (await fs.access(RUST_BASE_PATH).then(() => true).catch(() => false)) {
      const dataDocs = await scanDirectory(RUST_BASE_PATH);
    ...
    if (fileType && fileType !== 'all') {
      ...
      if (fileType === 'rust' || fileType === 'rs') targetType = 'rust';
...
router.get('/*path', async (req, res) => {
  try {
    ...
    let fullPath = path.join(RUST_BASE_PATH, requestedPath);
    
    // Security check - ensure the path is within the data directory
    const resolvedPath = path.resolve(fullPath);
    const resolvedBase = path.resolve(RUST_BASE_PATH);
    ...
    if (fileInfo.fileType === 'markdown') {
      let allDocuments = [];
      if (await fs.access(RUST_BASE_PATH).then(() => true).catch(() => false)) {
        allDocuments = await scanDirectory(RUST_BASE_PATH);
      }
