...
const PYTHON_BASE_PATH = path.join(__dirname, '../../../data');
...
async function checkDataStructure() {
  try {
    ...
    if (await fs.access(PYTHON_BASE_PATH).then(() => true).catch(() => false)) {
      console.log('✓ Data folder found at:', PYTHON_BASE_PATH);
    } else {
      console.log('✗ Data folder not found at:', PYTHON_BASE_PATH);
    }
  } catch (error) {
...
async function scanDirectory(dirPath, basePath = PYTHON_BASE_PATH, maxDepth = 3, currentDepth = 0) {
  const items = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (['.py', '.md', '.txt', '.js', '.html', '.css'].includes(ext)) {
...
router.get('/', async (req, res) => {
  try {
    ...
    // Scan data directory
    if (await fs.access(PYTHON_BASE_PATH).then(() => true).catch(() => false)) {
      const dataDocs = await scanDirectory(PYTHON_BASE_PATH);
    ...
    if (fileType && fileType !== 'all') {
      ...
      if (fileType === 'py') targetType = 'python';
...
router.get('/*path', async (req, res) => {
  try {
    ...
    let fullPath = path.join(PYTHON_BASE_PATH, requestedPath);
    
    // Security check - ensure the path is within the data directory
    const resolvedPath = path.resolve(fullPath);
    const resolvedBase = path.resolve(PYTHON_BASE_PATH);
    ...
    if (fileInfo.fileType === 'markdown') {
      let allDocuments = [];
      if (await fs.access(PYTHON_BASE_PATH).then(() => true).catch(() => false)) {
        allDocuments = await scanDirectory(PYTHON_BASE_PATH);
      }
