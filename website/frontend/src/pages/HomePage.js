...
export default function HomePage() {
  ...
  const [rsFiles, setRsFiles] = useState([]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      getDocuments({ limit: 100, fileType: 'rust', ungrouped: true }),
    ]).then(([s, c, r, p]) => {
      setCategories(c.data);
      setRecent(r.data.documents);
      setRsFiles(p.data.documents);
  ...
  return (
    <div>
      ...
      <section className="hero fade-up">
        ...
        <p>
          I provide a curated collection of Rust algorithms, memory management guides, 
          and technical documentation designed for systems engineers.
        </p>
      ...
      <section style={{ marginBottom: '80px' }}>
        ...
        <div className="grid grid-bento">
          ...
          {rsFiles.map(doc => <DocCard key={doc.path} doc={doc} />)}
        </div>
      </section>
