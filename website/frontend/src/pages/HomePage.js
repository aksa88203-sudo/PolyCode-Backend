...
export default function HomePage() {
  ...
  const [cppFiles, setCppFiles] = useState([]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      getDocuments({ limit: 100, fileType: 'cpp', ungrouped: true }),
    ]).then(([s, c, r, p]) => {
      setCategories(c.data);
      setRecent(r.data.documents);
      setCppFiles(p.data.documents);
  ...
  return (
    <div>
      ...
      <section className="hero fade-up">
        ...
        <p>
          I provide a curated collection of C++ algorithms, data structures, 
      ...
      <section style={{ marginBottom: '80px' }}>
        ...
        <div className="grid grid-bento">
          ...
          {cppFiles.map(doc => <DocCard key={doc.path} doc={doc} />)}
        </div>
      </section>
